...
import { enforceSafety, extractScheduleWindow, extractStreetAddresses, fallbackAnalysis, type JobCategory, type PlannerAnalysis, type PlannerQuestion } from "../lib/planner";
...
type PlanKey = "budget" | "recommended" | "complete";
type Answers = Record<string, string | boolean>;
type GoogleRoute = { distanceKm: number; trafficMinutes: number; source: string; legs: Array<{ from: string; to: string; distanceKm: number; trafficMinutes: number }> };
type ServiceAssignment = { title: string; executors: string; tasks: string; handoff: string; arrival: string; departure: string };
...
function assignmentCoversTask(assignment: ServiceAssignment, taskSequence: number) {
...
function serviceForTask(assignments: ServiceAssignment[], taskSequence: number) {
...
function formExecutorTeam(analysis: PlannerAnalysis, plan: PlanKey) {
...
function recalculateJob(analysis: PlannerAnalysis, answers: Answers) {
...
function optionsFor(analysis: PlannerAnalysis, answers: Answers): PlanOption[] {
...
function Question({ question, value, onChange, onTextCommit, busy }: { question: PlannerQuestion; value: string | boolean | undefined; onChange: (value: string | boolean) => void; onTextCommit: () => void; busy: boolean }) {
...
function addMinutesToSchedule(schedule: string, minutes: number) {
...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {stage === 1 && analysis && <section className="screen">
      ...
      <div className="analysis-intro"><div className="ai-orb">✦</div><div><div className="eyebrow">DONEEO UNDERSTOOD THE REQUEST</div><h2>{analysis.title}</h2></div></div>
      ...
      <section className="equipment-check illustrated-section">
        <img className="section-comic resource-comic" src="/brand/doneeo-comic-resources.svg" alt="A prepared executor checking tools, supplies and safety equipment" />
