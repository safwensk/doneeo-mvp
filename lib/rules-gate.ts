...
export function applyDoneeoRulesGate(input: PlannerAnalysis): PlannerAnalysis {
  ...
  const regulatedTrade = plumbingWork || electricalWork || /(electrical panel|new circuit|rewir|gas line|gas appliance|plumb(?:ing)? connection|water line|load-bearing|structural wall|fire alarm|refrigerant)/i.test(source) || /new or modified/.test(connectionScope) || /new or modified/.test(plumbingScope) || /new or modified/.test(rangeConnectionScope) || /hardwired|circuit|panel|wiring/.test(electricalScope) || /gas|dual fuel/.test(applianceEnergy) || answers.utilities === true || answers.regulated === true;
